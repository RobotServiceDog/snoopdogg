...
CallbackReturn ApriltagDetectionLifecycleNode::on_configure(const rclcpp_lifecycle::State &)
{
    ...
    apriltag_pose_msg_.x = 0.0;
    apriltag_pose_msg_.y = 0.0;
    apriltag_pose_msg_.theta = 0.0;
    ...
    if (environment_ == 0) {
        // Change MutuallyExclusive to Reentrant
...
void ApriltagDetectionLifecycleNode::image_callback(const sensor_msgs::msg::Image::ConstSharedPtr msg)
{
    ...
    RCLCPP_INFO(get_logger(), "Received image frame for detection (timestamp: %u.%u)", msg->header.stamp.sec, msg->header.stamp.nanosec);
...
void ApriltagDetectionLifecycleNode::camera_info_callback(const sensor_msgs::msg::CameraInfo::ConstSharedPtr msg)
{
    ...
    camera_info_sub_.reset(); // This destroys the subscription object
}
...
{   
    RCLCPP_INFO(get_logger(), "Processing frame for detection (size: %dx%d)", frame.cols, frame.rows);
    if (!apriltag_pose_pub_ || !apriltag_pose_pub_->is_activated()) {
        return;
    }

    RCLCPP_INFO(get_logger(), "Publisher is active, proceeding with detection...");
    if (frame.empty()) {
        return;
    }
    RCLCPP_INFO(get_logger(), "Frame is valid, checking resolution and camera info...");
    ...
    RCLCPP_INFO(get_logger(), "Checking camera info availability...");
    ...
    RCLCPP_INFO(get_logger(), "Camera info is available, proceeding with AprilTag detection...");
    cv::Mat gray_image;
    if (frame.channels() == 1) {
        gray_image = frame;
        ...
        RCLCPP_INFO(
            get_logger(), "Unsupported image channel count: %d", frame.channels());
        return;
    }
    ...
    if (ids.empty()) {
        RCLCPP_INFO(get_logger(), "No AprilTags detected in the current frame.");
        apriltag_pose_msg_.x *= 0.5;
        apriltag_pose_msg_.y *= 0.5; 
        apriltag_pose_msg_.theta *= 0.5;
    ...
    if (target_tag_id_ >= 0) {
        ...
        if (it == ids.end()) {
            RCLCPP_INFO(get_logger(), "Target tag ID %d not found in detected tags.", target_tag_id_);
            apriltag_pose_msg_.x *= 0.5;
            apriltag_pose_msg_.y *= 0.5;
            apriltag_pose_msg_.theta *= 0.5;
            apriltag_pose_pub_->publish(apriltag_pose_msg_);
    ...
    if (rvecs.empty() || tvecs.empty()) {
        RCLCPP_INFO(get_logger(), "Pose estimation failed for detected tag ID %d.", ids.at(selected_index));
        apriltag_pose_msg_.x *= 0.5;
        apriltag_pose_msg_.y *= 0.5;  // Pose2D.y used for forward depth (z in camera frame).
        apriltag_pose_msg_.theta *= 0.5;
        apriltag_pose_pub_->publish(apriltag_pose_msg_);
        return;
    }
    ...
    apriltag_pose_msg_.y = -tvecs.front()[0];  // Pose2D.y used for forward depth (z in camera frame).
